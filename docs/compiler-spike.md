...
This bounded inspection does not prove where PCI enumeration applies visibility filtering, or that exposing only one character device is supported. It therefore does not supersede the pinned documentation's warning about one-node containers. A board reservation and BDF selector cannot establish that discovery leaves other active boards untouched. Physical execution remains blocked until that exact installed stack has externally verified initialization isolation. Offline compilation avoids this unresolved path by loading the descriptor and skipping physical mesh creation, with no device nodes exposed.
